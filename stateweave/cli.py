...
def cmd_report(args):
    ...
    lines = [
        ...
        "",
        f"Generated by `stateweave report`",
        "",
        "## Agents",
        "",
    ]
    ...
    lines.append("")
    lines.append(f"- **Adapters:** {len(ADAPTERS)} frameworks ({', '.join(sorted(ADAPTERS.keys()))})")
    lines.append(f"- **Version:** {stateweave.__version__}")
    lines.append("")
    lines.append("---")
    lines.append("*Share this report in PRs, Slack, or docs to show agent state at a point in time.*")
        ...
        if os.path.isfile(hook_path):
            with open(hook_path) as f:
                if HOOK_MARKER in f.read():
                    ...
                    return
            print("Warning: pre-commit hook already exists. Appending StateWeave hook.", file=sys.stderr)
...
def main():
    ...
    # report
    report_parser = subparsers.add_parser("report", help="Generate shareable markdown report of agent state")
    report_parser.add_argument("agent_id", nargs="?", default=None, help="Agent ID (all agents if omitted)")

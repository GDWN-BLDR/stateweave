...
def cmd_upgrade(args):
    ...
    try:
        ...
        if latest == current:
            ...
            print("     Run: pip install --upgrade stateweave")
    except Exception:
...
def cmd_env(args):
    ...
    if os.path.isdir(store_path):
        total_size = sum(
            os.path.getsize(os.path.join(dp, f)) for dp, _, fns in os.walk(store_path) for f in fns
        )
...
def cmd_report(args):
    ...
    lines = [
        f"# 🧶 StateWeave Report — {now}",
        "",
        "Generated by `stateweave report`",
        "",
        "## Agents",
        "",
    ]
    ...
    for aid in sorted(agents):
        ...
        try:
            history = store.get_history(aid)
            lines.append(f"- **Checkpoints:** {len(history)}")
            if history:
                latest = history[-1]
                if hasattr(latest, "label"):
                    lines.append(f"- **Latest:** {latest.label}")
                if hasattr(latest, "timestamp"):
                    lines.append(f"- **When:** {latest.timestamp}")
        except Exception:
            lines.append("- *Unable to read checkpoint history*")
        lines.append("")

    lines.append("## Environment")
    lines.append("")
    lines.append(
        f"- **Adapters:** {len(ADAPTERS)} frameworks ({', '.join(sorted(ADAPTERS.keys()))})"
    )
    lines.append(f"- **Version:** {stateweave.__version__}")
    lines.append("")
    lines.append("---")
    lines.append(
        "*Share this report in PRs, Slack, or docs to show agent state at a point in time.*"
    )
...
def cmd_hook(args):
    """Install git pre-commit hook for stateweave ci."""
    import os
    import stat
    ...
    HOOK_MARKER = "# stateweave-hook"
    HOOK_CONTENT = f"""#!/bin/sh
{HOOK_MARKER}
# StateWeave pre-commit hook — auto-runs state regression checks
echo "🧶 Running stateweave ci..."
if command -v stateweave >/dev/null 2>&1; then
    stateweave ci default --baseline-version 1 2>/dev/null
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
        echo "❌ stateweave ci failed — commit blocked"
        echo "   Run: stateweave why default"
        exit 1
    fi
    echo "✅ stateweave ci passed"
fi
"""
    ...
    if action == "install":
        if os.path.isfile(hook_path):
            with open(hook_path) as f:
                if HOOK_MARKER in f.read():
                    print("StateWeave hook already installed.")
                    return
            print(
                "Warning: pre-commit hook already exists. Appending StateWeave hook.",
                file=sys.stderr,
            )
        ...
        print("✅ StateWeave pre-commit hook installed!")
        print(f"   Path: {hook_path}")
        print("   Now every commit will run: stateweave ci")
        ...
        if os.path.isfile(hook_path):
            ...
            if HOOK_MARKER in content:
                # Remove the stateweave hook portion
                lines = content.split("\n")
                new_lines = []
                skip = False
                for line in lines:
                ...
                if cleaned:
                    ...
                    os.remove(hook_path)
                print("✅ StateWeave pre-commit hook removed.")
            else:
                print("No StateWeave hook found in pre-commit.")
        else:
        ...
        if os.path.isfile(hook_path):
            ...
            if HOOK_MARKER in content:
                ...
                print("⚠️  pre-commit hook exists but no StateWeave hook")
        else:
            print("❌ No pre-commit hook installed")
            print("   Run: stateweave hook install")
...
def cmd_try(args):
    ...
    if source == target:
        print(
            f"\n  Source and target are the same ({source}). Pick different frameworks.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_benchmark(args):
    ...
    for r in results:
        ...
        print(
            f"  {r['framework']:<20} {r['messages']:<10} {r['memory']:<10} {time_str:<8} {status}"
        )
    ...
    if passed_count < total:
        print(
            f"     Partial fidelity on {total - passed_count} adapters — see tier docs for details"
        )
    print()
...
def main():
    ...
    migrate_parser.add_argument(
        "--agent", "-a", dest="agent_id", default="default", help="Agent ID"
    )
    ...
    benchmark_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show per-framework details"
    )
    ...
    stats_parser = subparsers.add_parser(
        "stats", help="Show aggregate checkpoint and agent statistics"
    )
    stats_parser.add_argument("--store-dir", default=None, help="Checkpoint store directory")

    # completions
    comp_parser = subparsers.add_parser(
        "completions", help="Generate shell completions (bash/zsh/fish)"
    )
    comp_parser.add_argument("shell", choices=["bash", "zsh", "fish"], help="Shell type")

    # report
    report_parser = subparsers.add_parser(
        "report", help="Generate shareable markdown report of agent state"
    )
    report_parser.add_argument(
        "agent_id", nargs="?", default=None, help="Agent ID (all agents if omitted)"
    )
    report_parser.add_argument("--output", "-o", help="Save report to file")
    report_parser.add_argument("--store-dir", default=None, help="Checkpoint store directory")

    # hook
    hook_parser = subparsers.add_parser(
        "hook", help="Install/remove git pre-commit hook for stateweave ci"
    )
    ...
    clean_parser.add_argument(
        "--keep", type=int, default=5, help="Number of latest checkpoints to keep (default: 5)"
    )
    ...
    search_parser = subparsers.add_parser(
        "search", help="Search checkpoint history for a key or value"
    )
    ...
    if args.command == "version":
        ...
        from stateweave.completions import generate_bash, generate_fish, generate_zsh
